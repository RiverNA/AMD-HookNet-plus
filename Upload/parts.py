...
class ESCA(nn.Module):
    def __init__(self, in_channels, HW):
        super().__init__()
        self.query = nn.Conv2d(in_channels, in_channels, kernel_size=1, groups=in_channels)
        self.key = nn.Conv2d(in_channels, in_channels, kernel_size=1, groups=in_channels)
        self.value = nn.Conv2d(in_channels, in_channels, kernel_size=1, groups=in_channels)
        self.softmax = nn.Softmax(dim=-1)
        self.gamma = nn.Parameter(torch.zeros(1))
        self.merge = nn.Parameter(1.0 * torch.ones(HW, in_channels))
        self.embed = nn.Conv2d(in_channels, in_channels, kernel_size=1)

    def forward(self, x):
        B, C, H, W = x.shape
        Q = self.query(x).view(B, -1, H * W).permute(0, 2, 1).contiguous()
        K = self.key(x).view(B, -1, H * W)
        _, head_dim, _ = K.shape
        attention = self.softmax(torch.bmm(Q, K) / (head_dim ** 0.5))
        V = self.value(x).view(B, C, H * W).permute(0, 2, 1).contiguous()
        out = torch.bmm(attention, V).permute(0, 2, 1).contiguous().view(B, C, H, W)
        feats1 = self.gamma * out + x
        feats2 = feats1.permute(0, 2, 3, 1).contiguous().view(B, H * W, -1)
        merge = F.softmax(self.merge, dim=-1)
        feats3 = merge * feats2
        x = self.embed(feats3.permute(0, 2, 1).contiguous().view(B, -1, int(np.sqrt(H * W)), int(np.sqrt(H * W))))

        return x


class T_Upsample(nn.Module):
    def __init__(self, in_channels, out_channels, HW, kernel_size=3):
        super().__init__()
        self.up = nn.ConvTranspose2d(in_channels, in_channels, kernel_size=2, stride=2)
        self.conv = Conv_block(in_channels, out_channels, kernel_size, mid_channels=out_channels // 2)
        self.catconv = Conv_block(out_channels * 2, out_channels, kernel_size, mid_channels=out_channels // 2)
        self.esca = ESCA(in_channels, HW)
    ...
    def forward(self, x, y, z):
        ...
        pre = F.normalize(output, p=2, dim=1)
...
class SwinTransformerSys(nn.Module):
    ...
    def forward_up_features(self, x, x_downsample):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = layer_up(x)
                B, hw, C = x.shape
                xx = x.view(B, int(np.sqrt(hw)), int(np.sqrt(hw)), C)
                xx = xx.permute(0, 3, 1, 2)
                c_outhooks.append(xx)
    ...
    def forward(self, x):
        ...
        x, c_outhooks = self.forward_up_features(x, x_downsample)
