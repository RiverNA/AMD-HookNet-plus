...
def eval_net(model, loader, device):
    ...
    n_val = len(loader)
    iou_ratio = 0
    test_save = os.path.join(os.environ['TMPDIR'], 'valid_results')
    ...
    piou = iou_ratio / n_val
    groundtruth = os.path.join(os.environ['TMPDIR'], 'ori')
    whole_save = os.path.join(os.environ['TMPDIR'], 'whole_results')
